...
class MarkPrice {
public:
    explicit MarkPrice(std::string  symbol, const double& price, const double& ask, const double& last, const uint64_t volume) : _symbol(std::move(symbol)), _bid(price), _ask(ask), _last(last), _volume(volume) {
